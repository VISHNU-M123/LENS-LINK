...
const updateService = async (req, res) => {
    try {
        const photographerId = req.photographerId
        const {serviceId, serviceName, serviceDescription, minPrice, maxPrice, currency, serviceDuration, serviceStatus} = req.body
        ...
        const existingService = await serviceModel.findOne({serviceName:serviceName, photographer:photographerId})
        ...
        res.status(200).json({success:true, message:'Service updated successfully', updatedService:service})
    } catch (error) {
        res.status(500).json({success:false, message:error.message})
    }
}
